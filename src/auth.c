...
static uint8_t b2a[64]  = { 0x30, 0x31, 0x32, 0x33, 0x34, 0x35, 0x36, 0x37, 0x38, 0x39, 0x41, 0x42, 0x43, 0x44, 0x45, 0x46,   /* 0 ... 9, A ... F */
                            ...
                            0x6d, 0x6e, 0x6f, 0x70, 0x71, 0x72, 0x73, 0x74, 0x75, 0x76, 0x77, 0x78, 0x79, 0x7a, 0x3c, 0x3e }; /* m ... z, < , >   */

// mapping ascii 0x30 ...0x7f back to 6 bit binary, invalids are mapped to 0xff
static uint8_t a2b[256] = { 0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0xff, 0xff, 0x3e, 0xff, 0x3f, 0xff,   /* 0x30 ... 0x3f */
...
int ascii_to_bin (uint8_t *out, uint8_t *in) {
    ...
    for(in_count = 0; in_count < strlen(in); in_count++) {
        ...
        if((in[in_count] > 0x30) && (in[in_count] < 0x80))
            buf |= a2b[in[in_count] - 0x30];
/*        else
            // !!!!!!!!!
            // !!! log message ???
            // !!! not if called from keygen tool because it shan't report errors to stdout
*/
        bit_count += 6;
