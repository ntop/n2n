...
static void register_with_new_peer(n2n_edge_t *eee,
  ...
  /* NOTE: pending_peers are purged periodically with purge_expired_pending_peers */
  if (scan == NULL) {
...
int run_edge_loop(n2n_edge_t * eee, int *keep_running) {
  ...
  *keep_running = 1;
  ...
  while(*keep_running) {
    ...
    numPurged += purge_expired_pending_peers(&eee->pending_peers, &last_purge_pending, PURGE_PENDING_PEERS_FREQUENCY);
